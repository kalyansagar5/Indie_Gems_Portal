pipeline {
    ...
    stages {
            ...
            steps {
                dir("${WORK_DIR}") {
                    sh '''
                        ...
                        # Build frontend with suppressed Chakra UI warnings
                        VITE_SUPPRESS_USE_CLIENT_WARNING=true npm run build
        ...
        stage('Verify Deployment') {
            steps {
                sh '''
                    # Wait for rollout to complete; fail stage if deployment fails
